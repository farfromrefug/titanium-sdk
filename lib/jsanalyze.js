...
exports.analyzeJs = function analyzeJs(contents, opts) {
	var ast,
		...
		results = {
			original: contents,
			contents: contents,
			symbols: contents.match(/["'](Ti|Titanium)\.[\w\.]+['"]/gi) || []
		};

	opts || (opts = {});
	results.symbols = results.symbols.filter(onlyUnique).map(function (s) { return s.slice(1,-1).split('.').slice(1).join('.') });
	...
	results.symbols = results.symbols.concat(Object.keys(symbols));
